...
export function extractQuestionCandidates(jobId, pages, allTopics = [], diagramMap = {}) {
  ...
  for (let i = 0; i < qMatches.length; i++) {
    ...
    const stemAndOptionsText = `${parsed.questionText || ''} ${Object.values(parsed.options || {}).join(' ')}`;
    const stemDiagramReferenced = Boolean(
      stemAndOptionsText.includes('imgur') ||
      stemAndOptionsText.includes('<img') ||
      stemAndOptionsText.includes('![') ||
      stemAndOptionsText.includes('*[Diagram:') ||
      /\b(?:shown in (?:the )?(?:figure|diagram|circuit)|given (?:in the )?(?:figure|circuit|diagram|graph)|as per (?:the )?(?:figure|diagram)|in the given circuit|circuit shown|graph below|plot below|reactions are respectively|titration plot)\b/i.test(stemAndOptionsText)
    );
    const hasSolutionDiagram = Boolean(
      solutionText && (
        solutionText.includes('```mermaid') ||
        /\b(?:diagram|circuit|schematic):/i.test(solutionText) ||
        /\bgraph\s+(?:LR|TD|TB|RL)\b/i.test(solutionText)
      )
    );
    ...
    candidates.push({
      ...
      has_diagram: stemDiagramReferenced,
      diagram_referenced: stemDiagramReferenced,
      has_solution_diagram: hasSolutionDiagram,
