...
async function ensureLocalSourcePdf(job) {
  ...
  const storagePath = job.source?.storage_path || job.source_storage_path || job.source?.metadata?.storage_path;
  if (storagePath) {
    try {
      const buffer = await downloadSourcePdf(storagePath);
  ...
  if (storagePath) {
    candidateNames.add(path.basename(storagePath));
  }
  ...
  if (process.platform === 'win32') {
    baseDirs.unshift('C:\\Users\\Admin\\Desktop\\JEE _Mains');
  }
...
export const contentService = {
  ...
  async cropPdfDiagram(jobId, pageNumber, rect, dpi = 300) {
    ...
    const pythonScript = path.resolve(__dirname, './pdf-renderer.py');
    const tempCropDir = path.resolve(__dirname, '../../../public/uploads/questions');
    if (!fs.existsSync(tempCropDir)) {
      fs.mkdirSync(tempCropDir, { recursive: true });
    }
    const tempCropPath = path.resolve(tempCropDir, `crop_${Date.now()}_${randomBytes(4).toString('hex')}.png`);
    ...
    try {
      ...
      if (!fs.existsSync(tempCropPath)) {
        throw applicationError('Cropped image file was not generated by renderer', 500);
      }
      ...
      if (execErr.statusCode) throw execErr;
