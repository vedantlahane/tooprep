...
// POST /api/evaluations — create evaluation, auto-assemble questions per §5.5
router.post('/', async (req, res) => {
  try {
    ...
    // If not enough after exclusion, fall back to full pool
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (evaluation.ended_at) {
      ...
      return res.json({ evaluation, attempts, questions });
    }
...
router.post('/:id/attempts', async (req, res) => {
  try {
    ...
    // Don't reveal correct_answer during evaluation
